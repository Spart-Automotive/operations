...
<template>
  <master
    :crumbs = "['brands', init ? init.name : 'Loading..']"
...
export default {
  ...
  computed: {
    ...
    avatarUrl() {
      return `${config.baseURL}/brand/${this.$route.params.id}/image`
    },
    avatar() {
      return this.init && this.init.image
        ? `${config.baseURL}/brand/${this.$route.params.id}/image`
        : null
    }
  },
  watch: {
  ...
  methods: {
    getInit() {
      this.$axios.get(`/brand/${this.$route.params.id}`).then(res => {
        this.init = res.data
